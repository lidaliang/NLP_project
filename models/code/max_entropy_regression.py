...
FREQ_DIST_FILE = 'test-structured-service-linear-freqdist.pkl'
BI_FREQ_DIST_FILE = 'test-structured-service-linear-freqdist-bi.pkl'
TRAIN_PROCESSED_FILE = 'test-structured-service-linear.csv'
TEST_PROCESSED_FILE = 'test-2-processed.csv'
# TEST_PROCESSED_FILE = 'test-structured-service-linear.csv'
TRAIN = False
UNIGRAM_SIZE = 15000
...
def extract_features(tweets, batch_size=50, test_file=True, feat_type='presence'):
...
if __name__ == '__main__':
    ...
    if TRAIN: # train = True is required
    ...
    best_val_acc = 0.0
    #print(val_tweets[0][1])
    #val_set_X, val_set_y = extract_features(val_tweets, batch_size=10)
    #print(val_set_y)

    # train data on existing features
    for j in range(nb_epochs):
        i = 1
        for training_set_X, training_set_y in extract_features(train_tweets, feat_type=FEAT_TYPE, batch_size=batch_size, test_file=False): # is the correct values returned?
