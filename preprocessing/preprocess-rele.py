...
def preprocess_df(structured_file_name):
    ...
    service_file_name = sys.argv[1][:-4] + '-service-linear-rele.csv'
    ...
    with open(structured_file_name, 'r',encoding='utf-8') as csv:
        ...
        for i, line in enumerate(lines):
            if i==0: continue
            line = line.split(',')
            post_id, content, n_likes, sentiment, rele_hand = line[0],line[10],line[5],line[8],line[8]
            rele_hand = int(float(rele_hand))
            if rele_hand > 2: rele_hand = 1
            else: rele_hand = 0
            processed_content = preprocess_tweet(content)
            save_to_file.write('%s,%s,%s\n' % (post_id,rele_hand,processed_content))
