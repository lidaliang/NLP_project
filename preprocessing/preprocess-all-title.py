...
def preprocess_df(structured_file_name):
    ...
    service_file_name = sys.argv[1][:-4] + '-title-linear.csv'
    ...
    with open(structured_file_name, 'r',encoding='utf-8') as csv:
        ...
        for i, line in enumerate(lines):
            if i==0: continue
            line = line.split(',')
            post_id, content, n_likes, sentiment_hand, relevance = line[0],line[12],line[5],line[9],line[8]
            sentiment_hand = int(float(sentiment_hand))
            if sentiment_hand > 3: sentiment = 1
            else: sentiment = 0
            processed_content = preprocess_tweet(content,use_stemmer = False)
            save_to_file.write('%s,%s,%s\n' % (post_id,n_likes,processed_content))
